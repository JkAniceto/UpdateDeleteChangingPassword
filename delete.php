...
<?php
    if(isset($_GET['deleteid'])){
        $id=$_GET['deleteid'];
          //initialization
        $query1 = "DELETE from user_tb WHERE id='$id'";
        $query2 = "DELETE from userInfo_tb WHERE id='$id'";
        if(!Query($query1))
          echo "fail to save to database";
        elseif(!Query($query2))
          echo "fail to save to database";
        else
          echo ("<script>window.location.replace('accountManagement.php'); alert('Delete Success');</script>");
